...
task :combine_css do
  ...
  system "cat #{styles_dir}/font-awesome.min.css #{styles_dir}/screen.css > #{styles_dir}/all.css"
end
...
task :combine_js do
  ...
  system "cat #{scripts_dir}/libs/jquery.min.js #{scripts_dir}/libs/jHXR.js #{scripts_dir}/libs/swfobject > #{scripts_dir}/all.js"
end
...
task :minify_js do
  ...
  smap_root_option = "--source-map-root http://kyleboon.org"
...
task :gzip_css do
  puts "## GZipping CSS"
  system 'find public/ -type f -name \*.css -exec gzip -9 {} \;'
  # Batch rename .html.gz to .html
  Dir['**/*.css.gz'].each do |f|
...
task :gzip_js do
  puts "## GZipping JS"
  system 'find public/ -type f -name \*.js -exec gzip -9 {} \;'
  # Batch rename .html.gz to .html
  Dir['**/*.js.gz'].each do |f|
...
desc "Generate jekyll site"
task :generate => [:sass, :update_asset_versions, :orig_generate, :gzip]
